...
{
    public static function ipsumWords($nwords){
        $text = '';
        $wordCount = 0;
        while ($wordCount < $nwords) {
            $nsentences = random_int(3, 8);
            $sentences = [];
            for($s = 0; $s < $nsentences && $wordCount < $nwords; ++$s) {
                $frags = [];
                $commaChance = .33;
                while(true) {
                    $nwordsInSentence = min(random_int(3, 15), $nwords - $wordCount);
                    $words = self::random_values(self::$lorem, $nwordsInSentence);
                    $wordCount += $nwordsInSentence;
                    $frags[] = implode(' ', $words);
                    if(self::random_float() >= $commaChance || $wordCount >= $nwords) {
                        break;
                    }
                    $commaChance /= 2;
                }
    ...
    public static function ipsumParagraph($nparagraphs) {
        $paragraphs = [];
